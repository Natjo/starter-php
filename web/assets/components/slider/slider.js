function Slider(slider) {
  ...
  const updateTimeline = () => {
    ...
    const progress = s.disabled || scrollableWidth <= 0 ? 1 : clamp(content.scrollLeft / scrollableWidth, 0, 1);
    timelineEl.style.setProperty("--slider-progress", progress);
  };
  const updateControls = () => {
    btnPrev === null || btnPrev === void 0 || btnPrev.setAttribute("aria-disabled", s.index <= 0);
    btnPrev === null || btnPrev === void 0 || btnPrev.classList[s.disabled ? "add" : "remove"]("hide");
    btnNext === null || btnNext === void 0 || btnNext.setAttribute("aria-disabled", s.index === s.maxIndex);
    btnNext === null || btnNext === void 0 || btnNext.classList[s.disabled ? "add" : "remove"]("hide");
  };
  ...
  const goToCurrent = () => {
    ...
    animateTo(s.index === s.maxIndex ? s.contentWidth - s.viewportWidth : s.itemOffsets[s.index] - s.left);
  };
  const syncIndex = () => {
    if (s.disabled || !s.visible) return;
    const scrollLeft = content.scrollLeft;
    const nextIndex = Math.trunc((scrollLeft + (s.itemWidth / 2 + s.gap)) / s.itemStep);
    const clampedIndex = scrollLeft + s.viewportWidth >= s.contentWidth ? s.maxIndex : clamp(nextIndex, 0, s.maxIndex);
  ...
  api.add = () => {
    ...
    paginationEl === null || paginationEl === void 0 || paginationEl.addEventListener("click", onPaginationClick);
  ...
  api.remove = () => {
    ...
    paginationEl === null || paginationEl === void 0 || paginationEl.removeEventListener("click", onPaginationClick);
