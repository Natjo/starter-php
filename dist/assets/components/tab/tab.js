class Tab {
  ...
  setSelectedTab(currentTab, setFocus = true) {
    ...
    for (let i = 0; i < this.tabs.length; i += 1) {
      ...
      if (currentTab === tab) {
        var _this$tabpanels$i;
        tab.setAttribute('aria-selected', 'true');
        tab.removeAttribute('tabindex');
        (_this$tabpanels$i = this.tabpanels[i]) === null || _this$tabpanels$i === void 0 || _this$tabpanels$i.classList.remove('is-hidden');
        if (setFocus) {
          tab.focus();
        }
      } else {
        var _this$tabpanels$i2;
        tab.setAttribute('aria-selected', 'false');
        tab.tabIndex = -1;
        (_this$tabpanels$i2 = this.tabpanels[i]) === null || _this$tabpanels$i2 === void 0 || _this$tabpanels$i2.classList.add('is-hidden');
      }
    }
  }
