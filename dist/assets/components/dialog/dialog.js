...
const FOCUSABLE_SELECTOR = ['a[href]:not([disabled])', 'button:not([disabled])', 'textarea:not([disabled])', 'input:not([disabled])', 'select:not([disabled])', '[tabindex]:not([tabindex="-1"])'].join(", ");
...
const focusDialogDefault = dialog => {
  if (!dialog) return;
  const preferred = dialog.querySelector("[autofocus]") || dialog.querySelector("[data-dialog-focus]") || dialog.querySelector("[data-dialog-close]") || dialog.querySelector("button, [href], input, select, textarea, [tabindex]:not([tabindex='-1'])");
